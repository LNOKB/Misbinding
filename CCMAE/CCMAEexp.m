...
try
    ...
    for nowblock = 1:length(blockmat)
        ...
        for nowtrial = 1:length(trialmat) %40
            ...
            if exist('CCMAEexp.csv', 'file') ~= 2
                fid = fopen('CCMAEexp.csv', 'wt');
                fprintf(fid, 'Subnum,nowblock,nowblocktype,nowtrial,ind_up_color,test_color,testspeed,keypress,opposite_to_ind_response,RT\n,sospeed');
                fclose(fid);
            end

            fid = fopen('CCMAEexp.csv','at');
